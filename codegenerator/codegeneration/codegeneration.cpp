...
string getAllVariables(Node* currentNode, int &varNr)
{
    string result;

    for (unsigned long i = 0; i < currentNode->getNodes().size(); ++i)
    {
        result += getAllVariables(currentNode->getNodes().at(i), varNr);
    }
    if(currentNode->getType() == VARIABLE && currentNode->getParent()->getType()!= NV && currentNode->getParent()->getType()!= OR && currentNode->getParent()->getType()!= AND && currentNode->getParent()->getParent()->getType()!= ELSE && currentNode->getParent()->getParent()->getType()!= THEN && currentNode->getParent()->getType()!= IF){
        
        bool found = false;

        for(int i = 0; i < uniqueNames.size(); i++)
        {
            if(uniqueNames[i] == currentNode->getData())
            {
                found = true;
                break;
            }
        ...
        {
            varNr++;
            result = generateVariable(currentNode, result);
            uniqueNames.push_back(currentNode->getData());
        }
    }
...
int getFlowNumber(Node* currentNode){
    int flowNumber = 0;
    
    for (unsigned long i = 0; i < currentNode->getNodes().size(); ++i)
    {
        flowNumber += getFlowNumber(currentNode->getNodes().at(i));
    }

    if(currentNode->getType() == PARAGRAPH && currentNode->getParent()->getType()!=THEN && currentNode->getParent()->getType()!=ELSE)
    {
        bool flag = false;
        int number = 0;
        for(int j=0; j<currentNode->getNodes().size(); j++){
            if(currentNode->getNodes().at(j)->getData() == "¤"){
                cout<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
                flag = true;
                number++;
            }
    ...
    else if(currentNode->getType() == SP)
    {
        return 1;
    }
...
string traverseGenerate(Node* currentNode, string vars, string guid, int &varNr, int flowNr)
{
    string result;

    for (unsigned long i = 0; i < currentNode->getNodes().size(); ++i)
    {
        result += traverseGenerate(currentNode->getNodes().at(i), vars, guid, varNr, flowNr);
    }
    if(currentNode->getType() == DOCUMENT){
        result = generateDoc(currentNode, result, vars, guid, flowId, flowNr, varNr);
    }
    if(currentNode->getType() == PARAGRAPH)
    {
        result = generateParagraph(currentNode, result, flowId, flowNr, varNr, fonts, uniqueNames, tp, tableNumber, rowSetNumber);
    }
    if(currentNode->getType() == FONT)
    {
        generateFont(currentNode, result, fonts);
    }
    if(currentNode->getType() == AREA)
    {
        result = generateArea(currentNode, result, fonts);
    }
    if(currentNode->getType() == SP)
    {
        result = generateSp(currentNode, result, flowId, fonts, varNr, flowNr);
    }
